...
namespace blackboard::renderer
{
  ...
  bool init(app::Window &window, Api &renderer_api, const uint16_t width, const uint16_t height)
  {
    ...
    auto window_handle{native_window_handle(ImGui::GetMainViewport(), SDL_GetWindowID(window.window))};
    if (!window_handle)
    {
    ...
    bgfx_init.platformData.ndt = SDL_GetPointerProperty(SDL_GetWindowProperties(window.window), "SDL.window.x11.display", NULL);
