                ...
                if (normalIt != primitive.attributes.end())
                {
                    ...
                    fastgltf::iterateAccessorWithIndex<fastgltf::math::fvec3>(
                        ...
                        [&](fastgltf::math::fvec3 norm, std::size_t index)
                        {
                            ...
                            mesh.vertices[baseVertex + index].normal =
...
void Mesh::toSerialized(const std::vector<Mesh> &meshes, const std::filesystem::path &path)
{
    ...
    // write magic
    outFile.write(reinterpret_cast<const char *>(MESH_SERIALIZATION_MAGIC),
                  sizeof(MESH_SERIALIZATION_MAGIC));
    ...
    for (const auto &mesh : meshes)
    {
        ...
        outFile.write(reinterpret_cast<const char *>(mesh.material.baseColor.data()),
                      mesh.material.baseColor.size() * sizeof(float));
        outFile.write(reinterpret_cast<const char *>(mesh.material.emissionColor.data()),
                      mesh.material.emissionColor.size() * sizeof(float));
...
void Mesh::addCube(Mesh &mesh, float cx, float cy, float cz, float w, float h, float d,
                   bool projectedUVs)
{
