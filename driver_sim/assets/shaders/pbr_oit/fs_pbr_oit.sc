...
SAMPLER2D(s_momentsTex, 0);
SAMPLER2D(s_totalDepthTex, 1);
...
void main()
{
    ...
    float T = computeTransmittanceAtDepthFrom4PowerMoments(totalDepth, moments.yw, moments.xz, z, 3e-5, 0.25, vec4_splat(0.5));

	gl_FragData[0] = vec4(color.rgb * T, color.a * T);
}